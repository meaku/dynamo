...
/**
 * set schemas to be used by transformation
 * @param schemas
 */
...
DynamoDb.prototype.batchWriteItem = function (params, callback) {
        ...
        Object.keys(params.RequestItems).forEach(function (table) {
            ...
            params.RequestItems[table].forEach(function (request) {
                ...
                if (request.DeleteRequest) {
                    request.DeleteRequest.Key = self.transform.to(table, request.DeleteRequest.Key);
                }

                if (request.PutRequest) {
                    request.PutRequest.Item = self.transform.to(table, request.PutRequest.Item);
                }
            });
        });
    }
    ...
    return bindCallback(
        nodefn.call((this.client.batchWriteItem).bind(this.client), params),
        callback
    );
};
...
DynamoDb.prototype.batchGetItem = function (params, callback) {
    ...
    if (this.transform) {
        ...
        Object.keys(params.RequestItems).forEach(function (table) {
            ...
            params.RequestItems[table].Keys = params.RequestItems[table].Keys.map(function (key) {
                return self.transform.to(table, key);
            });
        });
    }
    ...
    return bindCallback(
        nodefn.call((this.client.batchGetItem).bind(this.client), params)
            .then(function (res) {
                ...
                if (self.transform) {
                    ...
                    Object.keys(res.Responses).forEach(function (table) {
                        ...
                        res.Responses[table] = res.Responses[table].map(function (item) {
                            return self.transform.from(table, item);
                        });
                    });
                }
 ...
 * query
 *
 * TODO maybe resolve with (res.Items, res)
 *
 * @param {Object} params
 * @param {Function} callback
 * @returns {Promise}
            ...
            .then(function (res) {
                ...
                if (res.Items && self.transform) {
                    res.Items = res.Items.map(function (item) {
                        return self.transform.from(params.TableName, item);
                    });
                }
...
//batchWriteStream
DynamoDb.prototype.batchWriteStream = function(params) {
    return new streams.BatchWrite(this.client, this, params, this.transform);
};
